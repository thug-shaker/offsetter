...
macro_rules! offset_export {
    ...
    (@guard ($current_offset:expr,) -> {$(#[$attr:meta])* struct $name:ident $(($amount:expr, $id:ident: $ty:ty))*}) => {
        ...
        $crate::paste::paste! {
            #[repr(C, packed)]
            $(#[$attr])* pub struct $name { $([<_pad $id>]: [u8;$amount], pub $id: $ty),* }
        }
    ...
    // add more fields
    (@guard ($current_offset:expr, $offset:literal $id:ident: $ty:ty, $($next:tt)*) -> {$($output:tt)*}) => {
        offset_export!(@guard ($offset + core::mem::size_of::<$ty>(), $($next)*) -> {$($output)* ($offset - ($current_offset), $id: $ty)});
    };

    // Entry points
    ($(#[$attr:meta])* struct $struct_name:ident {$($input:tt)*}) => {
        offset_export!(@guard (0, $($input)*) -> {$(#[$attr])* struct $struct_name});
    };
}
